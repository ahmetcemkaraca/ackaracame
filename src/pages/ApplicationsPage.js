...
import { Smartphone, Globe, Monitor, Code, ExternalLink, ArrowRight, AppWindow } from 'lucide-react';
...
const ApplicationsPage = () => {
  const { applications, loadApplications, loading, createApplication } = useProject();
  ...
  useEffect(() => {
    const seedApps = async () => {
      if (window.location.hash === '#seed' && applications.length === 0 && !loading) {
        ...
        const defaultApps = [
          ...
          },
          {
            ...
            type: 'iOS / Android',
            iconName: 'Globe',
            image: '', // Needs a real image
            description: 'Çocuk güvenliğine önem veren, aileler için tasarlanmış konum ve etkinlik keşif uygulaması. Güvenli rotalar, aile dostu mekanlar ve gerçek zamanlı bildirimler.',
            techStack: [
              { name: 'React Native', color: 'bg-blue-500' },
              { name: 'Firebase', color: 'bg-orange-500' },
              { name: 'Maps API', color: 'bg-green-500' }
            ],
            link: '/wheretogo/privacy',
            linkText: 'Gizlilik ve Şartlar'
          }
        ];
    ...
    seedApps();
  }, [applications.length, loading, createApplication, loadApplications]);
        ...
        {loading ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 lg:gap-8">
            {applications.map((app, index) => {
              const Icon = IconsMap[app.iconName] || AppWindow;
              return (
                <motion.article
                  key={app.id}
                  initial={{ opacity: 0, y: 20 }}
