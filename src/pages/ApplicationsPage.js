...
import { Smartphone, Globe, Monitor, Code, ExternalLink, ArrowRight, AppWindow, LayoutGrid } from 'lucide-react';
...
const ApplicationsPage = () => {
  const { applications, loadApplications, loading } = useProject();
        ...
        {loading ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 lg:gap-8">
            {applications.map((app, index) => {
              ...
              const heroImage = app.image || 'https://placehold.co/1600x900?text=ACKaraca.me';
              const isExternalLink = typeof app.link === 'string' && app.link.startsWith('http');
              return (
                <motion.article
                  key={app.id}
                  initial={{ opacity: 0, y: 20 }}
                ...
                >
                  ...
                  <div className="flex-1 p-6 pt-2 flex flex-col">
                    ...
                    <Link
                      ...
                      className="mt-3 inline-flex justify-center items-center w-full px-4 py-2.5 border border-neutral-200 dark:border-neutral-700 text-sm font-semibold rounded-lg text-neutral-700 dark:text-neutral-200 hover:border-primary hover:text-primary dark:hover:text-primary transition-colors"
                    >
                      Uygulama Detayı
